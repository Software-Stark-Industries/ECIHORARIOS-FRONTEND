...
var inscription = (function () {
    ...
    window.onload = async function(){
        ...
        //let subjectsPre = apiclient.getPlan();
        const semana = {'lunes':1,'martes':2,'miercoles':3,'jueves':4,'viernes':5,'sabado':6};
        let subjectsPre = JSON.parse(localStorage.getItem('planPreinscripcion'));
        console.log("SUBJECTS PRE INSCRIPCION: ",subjectsPre);         
        subjectsPre.forEach(materia => {
            //console.log("Materia: ",materia);
            showPreSubject(materia,semana);
    ...
    function redirectToAdminView(){
        ...
        function makeInscription(){
            ...
            const preinscription = JSON.parse(localStorage.getItem('planPreinscripcion'));
            console.log("Va a inscribir los horarios: ",preinscription);
            preinscription.forEach(materia => {
                console.log("Materia inscrita: ",materia);
        ...
        function showPreSubject(group,semana){
            ...
            //console.log("ayuda: ",document.getElementsByTagName("tbody"));
            let size = document.getElementsByTagName("tbody")[0].childNodes.length;                                    
            for(let i=0; i<size;i++){                                                                                
                if(document.getElementsByTagName("tbody")[0].childNodes[i].nodeName==="TR"){                    
                    let hora = document.getElementsByTagName("tbody")[0].childNodes[i].children[0].innerText;                                
                    if(hora == group.hInicio){
                        //console.log("GROUP DIAS: ",group.dias);
                        for(let j=0;j<group.dias.length;j++){                            
                            for(let k=0;k<group.dias.length;k++){       
                                //console.log("j: ",j);
                                //console.log("group.dias[j]: ",group.dias[j]);      
                                //console.log("semana[group.dias[j]]",semana[group.dias[j]]);
                                //console.log("elementos: ",document.getElementsByTagName("tbody")[0].childNodes[i].children[semana[group.dias[j]]]);
                                document.getElementsByTagName("tbody")[0].childNodes[i].children[semana[group.dias[j]]].innerText= `${group.grupo} - ${group.grupo} \n ${group.salon}`;
                            }  
                        }                                                                                
                    }
                }                                                
